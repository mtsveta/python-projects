...
def adaptive_4th_order(eps_rel, eps_abs, t_0, t_final, y_0, y, f_n, fprime_n, result_path):
    ...
    h_pred_array = np.append(np.array([]), np.array([]))
    ...
    while t < t_final:
        ...
        # reconstruct approximation y* of the 2nd order
        #y_star = y_n + h_star * f_n_val + h_star ** 2 / 2 * fprime_n_val
        y_star = y_n + h_star / 2 * f_n_val + h_star ** 2 / 8 * fprime_n_val
        ...
        eps = eps_abs * max(1, norm(y_star))

        # predicted time-step
        const = 1.1
        p = 2
        h_new = const * math.pow(math.fabs(eps / lte_star), 1 / (p + 1)) * h_star
        print('h* = %4.4e (h* pred/rej = %4.4e)\terr = %4.4e\tlte = %4.4e' % (h_star, h_new, err_star, lte_star))
        ...
        rho = h / h_star
        y_n1 = y_n + h * (1 - rho ** 2 + rho ** 3 / 2) * f_n_val \
        ...
        y_n1_3rd = y_n + h * (1 - rho ** 2) * f_n_val \
                   + h * rho ** 2 * f_star_val \
        ...
        h_pred_array = np.append(h_pred_array, np.array([h_new]))
    ...
    plot_results(t_array, yn_array, y_array, h_array, h_pred_array, err_array, result_path)
